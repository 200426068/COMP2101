...
myhostname=$(hostname)
interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}') #looking for interface name
lan_address=$(ip a s $interface_name|awk '/inet /{gsub(/\/.*/,"");print $2}') #flooking for a lan address
lan_hostname=$(getent hosts $(ip a s $interface_name)|awk '/inet /{gsub(/\/.*/,"");print $2}' | awk '{print $2}') #looking for lan hostname with getent hosts and variable $lan_address
external_ip=$(curl -s icanhazip.com) #achieveing external IP address from curl
external_name=$(getent hosts $external_ip| awk '{print $2}') #achieveing external ip by having a look into hosts file by $external_ip
router_address=$(ip r s default| cut -d ' ' -f 3) #looking for a default router address
router_hostname=$(getent hosts $router_address|awk '{print $2}') #looking for router hostname by getent hoste and $router_address

#we have to clean the script by moving output to different variables
cat <<EOF
Hostname        : $myhostname
